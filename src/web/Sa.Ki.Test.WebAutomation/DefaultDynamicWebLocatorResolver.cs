namespace Sa.Ki.Test.WebAutomation
{
    ...
    public class DefaultDynamicWebLocatorResolver : IDynamicWebLocatorResolver
    {
        ...
        public string ResolveDynamicLocatorValue(string locator, params (string parName, object parValue)[] values)
        {
            ...
            var locatorPlaceholders = new List<(int number, string varName, string format, string type)>();
            ...
            if (values.All(v => v.parName == null)) //by index mode
            {
                if (values.Length > locatorPlaceholders.Count)
                    throw new SakiWebAutomationException(nameof(IDynamicWebLocatorResolver), 
                        $"Error occurred during dynamic locator resolving." +
            ...
            else //by name mode
            {
                ...
                for (int i = 0; i < values.Length; i++)
                {
                    var (number, varName, format, type) = locatorPlaceholders.First(lp => lp.varName == values[i].parName);
                    var part = locatorParts[number];
                    var formatted = string.Format($"{{0:{format}}}", values[i].parValue);
                    var rp = _dynamicLocatorRegex.Replace(part, $"'{formatted}'");
                    locatorParts[number] = rp;
        ...
        public string ResolveDynamicLocatorValue(string locator, params object[] values)
        {
            return ResolveDynamicLocatorValue(locator,
                values.Select<object, (string parName, object parValue)>(v => (parName: null, parValue: v)).ToArray());
        }
    }
}
